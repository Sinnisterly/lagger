...
void getoffsets() {
    ifstream offsets;
    offsets.open(xor ("C:\\ls\\config.txt"));
    if (offsets.is_open()) {
        logs.AddLog(xor ("[!] LS config found"));
        logs.AddLog(xor ("\n[!] Loading LS config, wait..."));
        ...
        if (version[0] == s_version[0] && version[2] == s_version[2]) {
            ...
            logs.AddLog(xor ("\n[+] LS config loaded successfully\n"));
        }
        else {
            logs.AddLog(xor ("\n[!] You have a new version of LS.\nOld configuration removed to avoid bugs\n- Past installed version of LS: %s"), s_version);
            logs.AddLog(xor ("\n- Installed current version of LS: %s"), version);
            logs.AddLog(xor ("\n[!] Deleting the old LS configuration.."));
            remove("C:\\ls\\config.txt");
            logs.AddLog(xor ("\n[!] Creating a new LS config, wait..."));

            saveconf();

            logs.AddLog(xor ("\n[+] New LS config created successfully\n"));
        }
    }
    else {
        logs.AddLog(xor ("[-] LS config not found"));
        logs.AddLog(xor ("\n[!] Creating a LS config, wait..."));

        saveconf();

        logs.AddLog(xor ("\n[+] LS config created successfully\n"));
    }
}