...
void saveconf() {
    std::ifstream iff;
    iff.open(xor ("C:\\ls\\config.txt"));
    iff.close();
    if (!iff) {
        std::string directory = xor ("C:\\ls");
        CreateDirectory(directory.c_str(), NULL);
        directory.append(xor ("\\config.txt"));
    }
    remove(xor ("C:\\ls\\config.txt"));
    HANDLE h_file = CreateFile(xor ("C:\\ls\\config.txt"), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    CloseHandle(h_file);
    ...
    fstream f;
    f.open(xor ("C:\\ls\\config.txt"), ios::out);
